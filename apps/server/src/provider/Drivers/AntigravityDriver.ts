 ...
 *
 * The CLI is print-mode only, so the adapter spawns one process per turn and
 * the snapshot probe shells out to `agy --version` / `agy models`. Text
 * generation reuses the same binary through agy's structured-output mode.
 *
